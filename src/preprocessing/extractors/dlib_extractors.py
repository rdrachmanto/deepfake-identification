...
from src.preprocessing.extractors.detectors import DlibDetector
import src.preprocessing.extractors.utils as utils
import src.utils as coreutils


class DlibSampleExtractor(DlibDetector):
    def __init__(self, dataset_path: str, classes: list[str]) -> None:
        self.dataset_path = dataset_path
        self.classes = classes
        super().__init__()

    def extract(self, save_to: str, n_frame: int, cut_amount: float, seed: int, batch_size: int):
...
class DlibSeqExtractor(DlibDetector):
